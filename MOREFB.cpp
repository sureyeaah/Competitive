...
VL Convolution(VL a,VL b){
    ...
    VL c;
    for(auto i:C){
        c.push_back(((ll)round(i.r))%mod);
    }
    return c;
}
int expmod(ll a, int b) {
    ll ret = 1;
    for(;b;b >>= 1, a = (a*a) % mod) {
...
int solve(int val) {
    VL poly(1,1);
    FOR0(i, n) {
        VL cur(2); cur[1] = 1; cur[0] = expmod(val, s[i]);
        poly = Convolution(poly, cur);
    }
    return poly[n-k];
}
int main() {
    ...
    cin >> n >> k;
    FOR0(i, n) cin >> s[i];
    int x1 = ((1+five)*inv2) % mod, x2 = ((1-five+mod)*inv2) % mod;
    int ans = (solve(x1) - solve(x2)) % mod; if(ans < 0) ans += mod;
    cout << (ans*inv5)%mod;
}