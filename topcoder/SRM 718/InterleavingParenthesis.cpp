...
class InterleavingParenthesis {
    ...
    int countWays(string s1, string s2) {
        ...
        if((n1+n2)&1) return 0;
        FOR0(i, n1) cnt[0][i+1] = cnt[0][i] + (s1[i]=='(' ? 1 : -1);
        FOR0(i, n2) cnt[1][i+1] = cnt[1][i] + (s2[i]=='(' ? 1 : -1);
        FOR0(i,n1+1) if(cnt[0][i] >= 0) dp[i][0] = 1; else break;
        FOR0(i,n2+1) if(cnt[1][i] >= 0) dp[0][i] = 1; else break;
        FOR(i,1,n1+1) {
            FOR(j,1,n2+1) {
                if(cnt[0][i-1] )
            }
