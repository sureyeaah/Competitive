...
struct EllysSquareWords
{
    ...
    int getScore(string S)
    {
        ...
        int ans = 0, tot = 0;
        FOR0(i, 26) ans += cnt[i]*cnt[i];
        tot = ans;
        FOR0(i, 26) FOR0(j, 26) {if(i == j) continue;tot = max(tot, ans - (cnt[i]*cnt[i])- (cnt[j]*cnt[j]) + ((cnt[i] + cnt[j])*(cnt[i] + cnt[j])));}
        return tot;
    }
};
